...
function ActivityDetailsPanel() {
  ...
  // If context is not available or no activity is selected, don't render
  if (!context || !context.selectedActivity) {
    return null
  }

  const { selectedActivity, setSelectedActivity } = context
...
export function GlobalActivityDetailsPanel() {
  const pathname = usePathname()
  
  // Only render on trip pages where TripProvider is available
  if (!pathname.startsWith('/trip/')) {
    return null
  }
  
  return <ActivityDetailsPanel />
}
